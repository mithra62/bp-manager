...
class Accounts extends AbstractModel
{
     ...
     */
    public function createAccount(array $data, \Application\Model\Users $user, \PM\Model\Companies $company, \Application\Model\Hash $hash, \Application\Model\Settings $setting, \PM\Model\Options $option)
    {
        ...
        $user_roles = $user->roles->getAllRoles(array(
            'account_id' => 1
        ));
        ...
        if ($company_id) {
            ...
            $this->update('companies', $sql, array(
                'id' => $company_id
            ));
        }
    ...
    {
        $account_data = $this->getAccount(array(
            'id' => $account_id
        ));
        if ($account_data) {
    ...
    public function removeUserFromAccount($user_id, $account_id = false)
    {
        ...
        if ($this->remove('user_accounts', array(
            'user_id' => $user_id,
            'account_id' => $account_id
        ))) {
            $this->remove('user2role', array(
                'user_id' => $user_id,
                'account_id' => $account_id
            ));
            return true;
