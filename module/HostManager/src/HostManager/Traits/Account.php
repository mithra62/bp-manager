...
trait Account
{
    ...
    public function getAccountDetails($account_id)
    {
        ...
        $sql->join('companies', 'companies.id = settings.option_value', array(
            'company_name' => 'name'
        ));
        $sql->join('accounts', 'accounts.id = settings.account_id', array(
            'slug'
        ));
        $sql->join('users', 'accounts.owner_id = users.id', array(
            'email',
            'first_name',
            'last_name'
        ));
