...
class ActivityLog extends AbstractModel
{
    ...
    public function getUsersProjectActivity($id, $filter = FALSE, $limit = 20)
    {
        ...
        $sql->join(array(
            'pt' => 'project_teams'
        ), 'pt.project_id = a.project_id', array());
        
        $sql = $sql->join(array(
            'p' => 'projects'
        ), 'p.id = a.project_id', array(
            'project_name' => 'name',
            'project_id' => 'id'
        ), 'left');
        $sql = $sql->join(array(
            't' => 'tasks'
        ), 't.id = a.task_id', array(
            'task_name' => 'name'
        ), 'left');
        $sql = $sql->join(array(
            'n' => 'notes'
        ), 'n.id = a.note_id AND pt.project_id = n.project_id', array(
            'note_subject' => 'subject'
        ), 'left');
        $sql = $sql->join(array(
            'u' => 'users'
        ), 'u.id = a.user_id', array(
            'user_first_name' => 'first_name',
            'user_last_name' => 'last_name'
        ), 'left');
        $sql = $sql->join(array(
            'u2' => 'users'
        ), 'u2.id = a.performed_by', array(
            'performed_by_first_name' => 'first_name',
            'performed_by_last_name' => 'last_name'
        ), 'left');
        $sql = $sql->join(array(
            'b' => 'bookmarks'
        ), 'b.id = a.bookmark_id AND pt.project_id = b.project_id', array(
            'bookmark_name' => 'name'
        ), 'left');
        $sql = $sql->join(array(
            'f' => 'files'
        ), 'f.id = a.file_id', array(
            'file_name' => 'name'
        ), 'left');
        ...
        if ($filter && is_array($filter)) {
            ...
            if (array_key_exists('project_id', $filter) && is_numeric($filter['project_id'])) {
                $sql = $sql->where(array(
                    'a.project_id' => $filter['project_id']
                ));
            }
        }
